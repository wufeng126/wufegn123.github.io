...
type LimitPriceRow = {
  status?: string | null;
  limit_total_price?: string | number | null;
  actual_total_price?: string | number | null;
...
export async function GET(request: NextRequest) {
  ...
  // 计算统计数据
  const records = (data ?? []) as LimitPriceRow[];
  const stats = {
    total: records.length,
    draft: records.filter(record => record.status === '草稿').length,
    active: records.filter(record => record.status === '审核生效').length,
    invalidated: records.filter(record => record.status === '作废').length,
    totalLimitAmount: records.reduce((sum: number, record) => sum + parseFloat(String(record.limit_total_price ?? 0)), 0),
    totalActualAmount: records.reduce((sum: number, record) => sum + parseFloat(String(record.actual_total_price ?? 0)), 0),
    totalExcess: records.reduce((sum: number, record) => sum + parseFloat(String(record.excess_amount ?? 0)), 0)
  };
