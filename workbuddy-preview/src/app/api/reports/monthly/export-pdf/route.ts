...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      ...
      .select('id, report_month, project_scope, project_ids, template_type, generated_by, generated_at')
    ...
    const filteredSnapshots = accessibleProjects === null
      ? snapshots
      : snapshots.filter((snapshot: Record<string, unknown>) => {
          const projectIds = Array.isArray(snapshot.project_ids)
            ? snapshot.project_ids.map((projectId: unknown) => Number(projectId)).filter((projectId: number) => Number.isInteger(projectId))
            : [];
          return projectIds.some((projectId: number) => accessibleProjects.includes(projectId));
        });

    return NextResponse.json({ success: true, data: filteredSnapshots });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const projectScope = projectId && projectId !== 'all' ? 'selected' : accessibleProjects === null ? 'all' : 'selected';
      const projectIds = projectId && projectId !== 'all'
        ? projectId.split(',').map(id => Number(id)).filter(id => Number.isInteger(id))
        : accessibleProjects ?? [];
      await supabase
        .from('monthly_report_snapshots')
        .insert({
