...
type KpiRecord = Record<string, number>;
...
function extractKpiFromSnapshot(snapshot: SnapshotRecord | null | undefined): KpiRecord {
  if (!snapshot) return {};
  const summary = (snapshot.summary as SnapshotRecord | undefined) || snapshot;
  return {
    totalOutput: getNumber(summary.totalOutput || summary.totalIncome),
    totalReceived: getNumber(summary.totalReceived),
    totalCost: getNumber(summary.totalCost),
    totalProfit: getNumber(summary.totalProfit),
    profitRate: getNumber(summary.profitRate),
    paymentRate: getNumber(summary.paymentRate),
    monthIncome: getNumber(summary.monthIncome),
    monthCost: getNumber(summary.monthCost),
    monthProfit: getNumber(summary.monthProfit) || getNumber(summary.monthIncome) - getNumber(summary.monthCost),
    operatingProfit: getNumber(summary.operatingProfit),
    cashNetFlow: getNumber(summary.cashNetFlow),
    supplierUnpaid: getNumber(summary.supplierUnpaid),
    salaryUnpaid: getNumber(summary.salaryUnpaid),
    inServiceCount: getNumber(summary.inServiceCount),
    projectCount: getNumber(summary.projectCount),
  };
}

// Build side-by-side comparison of KPIs
function buildKpiComparison(kpi1: KpiRecord, kpi2: KpiRecord, month1: string, month2: string) {
  const fields = [
