...
ALTER TABLE IF EXISTS construction_log_progress_entries
  ADD COLUMN IF NOT EXISTS log_id INTEGER,
  ADD COLUMN IF NOT EXISTS project_id INTEGER,
  ADD COLUMN IF NOT EXISTS progress_task_id INTEGER,
  ADD COLUMN IF NOT EXISTS actual_progress NUMERIC(6,2) DEFAULT 0,
  ADD COLUMN IF NOT EXISTS completed_quantity NUMERIC(14,2),
  ADD COLUMN IF NOT EXISTS remark TEXT,
  ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT NOW(),
  ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 2026-08-14 (A1 修复): 核心业务表基线建表迁移（幂等，可重复执行）
-- 与 migrations/create_core_tables.sql 内容保持一致
-- ═══════════════════════════════════════════════════════════════════════════
