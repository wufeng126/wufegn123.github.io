...
export async function POST(request: NextRequest) {
  try {
    ...
    if (accessibleProjectIds !== null && projectId && !accessibleProjectIds.includes(projectId)) {
      return NextResponse.json({ error: '无权在该项目下新增生活费记录' }, { status: 403 });
    }
    ...
    const insertData = {
      ...
      receipt_item_id: normalizeId(body.receipt_item_id),
    ...
    await auditLog({
      ...
      details: { worker_id: workerId, project_id: projectId, year_month: yearMonth, amount },
      request,
    });
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { data, error } = await client
      ...
      .update({
        worker_id: workerId,
        project_id: normalizeId(body.project_id),
        year_month: yearMonth,
        allowance_date: allowanceDate,
        amount,
        payment_method: body.payment_method || '银行转账',
    ...
    await auditLog({
      ...
      resourceType: 'living_allowance',
      resourceId: id,
      details: { worker_id: workerId, year_month: yearMonth, amount },
      request,
    });
