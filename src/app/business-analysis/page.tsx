  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between gap-4 text-xs text-muted-foreground">
        <span>{label}</span>
        <span className="font-medium text-foreground/80">{formatWan(value)}</span>
      </div>
...
function BusinessOverviewPage() {
  ...
  const detailCards: DetailCard[] = [
    ...
    },
    {
      ...
      tone: 'bg-muted text-foreground/80 ring-border',
    },
  ];
  ...
  return (
    <main className="min-h-full bg-[var(--color-muted)] text-foreground">
      <div className="mx-auto max-w-[1480px] space-y-5 p-3 sm:p-4 md:p-6">
        ...
        <header className="flex flex-col gap-4 rounded-lg border border-border bg-card px-5 py-4 shadow-sm lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="mb-2 inline-flex items-center gap-2 rounded-md bg-muted px-2.5 py-1 text-xs font-medium text-muted-foreground">
              <Building2 className="h-3.5 w-3.5" />
              经营分析
            </div>
            <h1 className="text-2xl font-semibold tracking-normal text-foreground">公司经营总览</h1>
          ...
          <div className="grid grid-cols-2 gap-2 text-sm sm:grid-cols-4 lg:w-[620px]">
            <div className="rounded-md border border-border bg-muted/50 px-3 py-2">
              <div className="whitespace-nowrap text-xs text-muted-foreground">当前口径</div>
              <div className="mt-1 whitespace-nowrap font-semibold">真实汇总</div>
            </div>
            <div className="rounded-md border border-border bg-muted/50 px-3 py-2">
              <div className="whitespace-nowrap text-xs text-muted-foreground">结算来源</div>
              <div className="mt-1 whitespace-nowrap font-semibold">开票金额</div>
            </div>
            <div className="rounded-md border border-border bg-muted/50 px-3 py-2">
              <div className="whitespace-nowrap text-xs text-muted-foreground">应收口径</div>
              <div className="mt-1 whitespace-nowrap font-semibold">状态比例</div>
            </div>
            <div className="rounded-md border border-border bg-muted/50 px-3 py-2">
              <div className="whitespace-nowrap text-xs text-muted-foreground">质保账期</div>
          ...
          {metrics.map(item => {
            ...
            return (
              <article key={item.label} className="rounded-lg border border-border bg-card p-4 shadow-sm">
                ...
                <div className="mt-2 text-2xl font-semibold tabular-nums text-foreground">{item.value}</div>
                <div className="mt-2 min-h-[36px] text-xs leading-5 text-muted-foreground">{item.note}</div>
              </article>
            );
          })}
        </section>
        ...
        <section className="rounded-lg border border-border bg-card shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-4">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">按项目档案的状态付款比例，结合开票金额和回款实时计算当前应收、未收和账期。</p>
            </div>
          </div>
                  ...
                  projectLedger.map(row => {
                    ...
                    return (
                      <tr key={row.id} className="hover:bg-muted/50">
                        ...
                        <td className="px-4 py-4 text-muted-foreground">{row.effectiveStatus || row.status || '-'}</td>
                        <td className="px-4 py-4 text-muted-foreground">{formatRatio(row.paymentRatio)}</td>
                        ...
                        <td className="px-4 py-4 text-muted-foreground">{formatAging(row)}</td>
                  ...
                  <article key={row.id} className="rounded-lg border border-border bg-card p-4 shadow-sm">
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                      <div className="rounded-md bg-muted/50 p-3">
                        <div className="text-muted-foreground">结算金额</div>
                        <div className="mt-1 font-semibold text-foreground">{formatWan(toNumber(row.invoiceAmount))}</div>
                      </div>
                      <div className="rounded-md bg-muted/50 p-3">
                    ...
                    <div className="mt-3 flex items-center justify-between rounded-md bg-muted/50 px-3 py-2 text-xs text-muted-foreground">
                      <span>账期</span>
        ...
        <section className="grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
          <article className="rounded-lg border border-border bg-card shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-4">
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">用同一个口径看公司应该收多少、已经收多少，以及还要对外支付多少。</p>
              </div>
            ...
            <div className="grid gap-6 p-5 lg:grid-cols-2">
              <div className="rounded-lg border border-border/60 bg-muted/50 p-4">
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <div className="text-xs text-muted-foreground">产值结算 / 回款</div>
                </div>
              ...
              <div className="rounded-lg border border-border/60 bg-muted/50 p-4">
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <div className="text-xs text-muted-foreground">供应商 + 工资</div>
                </div>
                <div className="space-y-4">
          ...
          <article className="rounded-lg border border-border bg-card shadow-sm">
            <div className="border-b border-border/60 px-5 py-4">
              ...
              <p className="mt-1 text-xs text-muted-foreground">总览只负责告诉老板哪里需要看，细节仍回到原模块处理。</p>
            </div>
            <div className="grid gap-3 p-5 sm:grid-cols-2">
              {trendItems.map(item => (
                <div key={item.name} className="rounded-lg border border-border/60 bg-muted/50 p-4">
            ...
            {warnings.length > 0 && (
              <div className="border-t border-border/60 px-5 pb-5 pt-2">
                <div className="mb-2 text-xs font-medium text-muted-foreground">成本利润中心预警</div>
        ...
        <section className="grid gap-4 lg:grid-cols-4">
          {detailCards.map(card => {
            ...
            return (
              <a key={card.title} href={card.href} className="rounded-lg border border-border bg-card p-4 shadow-sm transition hover:border-blue-200 hover:shadow-md">
                ...
                <p className="mt-2 min-h-[72px] text-sm leading-6 text-muted-foreground">{card.desc}</p>
                <div className="mt-4 grid grid-cols-2 gap-2">
                  <div className="rounded-md bg-muted/50 px-3 py-2">
                    <div className="text-[11px] text-muted-foreground">核心指标</div>
                    <div className="mt-1 text-sm font-semibold">{card.primary}</div>
                  </div>
                  <div className="rounded-md bg-muted/50 px-3 py-2">
                    <div className="text-[11px] text-muted-foreground">需要关注</div>
