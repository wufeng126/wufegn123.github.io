...
export const aiDailyUsage = pgTable("ai_daily_usage", {
	...
	export const constructionLogs = pgTable("construction_logs", {
		...
		dailyGroupId: varchar("daily_group_id", { length: 64 }),
		...
		index("construction_logs_daily_group_id_idx").using("btree", table.dailyGroupId.asc().nullsLast().op("text_ops")),
...
export const constructionLogAttendance = pgTable("construction_log_attendance", {
	...
	workType: varchar("work_type", { length: 50 }),
	teamName: varchar("team_name", { length: 100 }),
	workHours: numeric("work_hours", { precision: 8, scale: 2 }).default('0'),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	...
	index("construction_log_attendance_project_worker_idx").using("btree", table.projectId.asc().nullsLast().op("int4_ops"), table.workerId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.logId],
